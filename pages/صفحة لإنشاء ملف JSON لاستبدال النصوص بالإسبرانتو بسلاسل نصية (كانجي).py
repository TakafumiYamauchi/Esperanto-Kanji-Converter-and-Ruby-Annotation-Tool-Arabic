...
# 
# 【概要】
#  このページでは、「エスペラント文の汉字(アラビア語)置換」で使う
#  最終的な JSON ファイル(合并3個JSONファイル)を生成する機能を提供します。
# 
# 【主な流れ】
#  1) CSVファイルを取り込み(エスペラント語根→汉字/アラビア語訳の対応)
...
#=====================================================================
st.set_page_config(
    page_title="أداة توليد ملفات JSON لاستبدال النص (الأحرف الصينية) في نصوص إسبرانتو",
    layout="wide"
)
st.title("إنشاء ملف JSON لاستخدامه في استبدال النص (الأحرف الصينية) في نصوص إسبرانتو")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("عرض شرح كيفية الاستخدام", expanded=True):
    st.markdown("""
    #### مقدمة
    في هذه الصفحة، بإمكانك إنشاء ملف JSON (حجمه قد يصل إلى 50 ميغابايت)
    لاستخدامه في استبدال النصوص في إسبرانتو ضمن الصفحة الرئيسية،
    ويمكنك تنزيله بعد ذلك.

    **خطوات الاستخدام**:
    1. قم برفع (أو استخدام الافتراضي) **ملف CSV** 
       (قائمة الجذور الإسبرانتية → الترجمة العربية) أو (جذور إسبرانتو → أحرف صينية).
    2. قم برفع (أو استخدام الافتراضي) **ملف JSON**
       (قواعد تجزئة الجذور أو النص المستبدل).
    3. اضغط على زر "إنشاء ملف JSON للاستبدال" للحصول
       على الملف الناتج وتنزيله.

    أدناه توجد بعض الملفات النموذجية التي تساعدك في فهم
    طريقة صياغة الإعدادات وتخصيصها.
    """)

#=====================================================================
# 3) サンプルファイル一覧 (折りたたみ)
#=====================================================================
with st.expander("ملفات نموذجية (للتنزيل)"):
    st.write("#### قائمة الملفات النموذجية")

    st.markdown("""
    **ملف CSV نموذجي رقم 1 (قائمة الجذور الإسبرانتية والترجمة العربية + تعليقات روبي)**  
    هذا ملف CSV يربط بين الجذور الإسبرانتية والترجمة العربية، مع إمكانية وجود تعليقات روبي في كل سطر.  
    برفع ملف CSV بهذا التنسيق، ستتولى الأداة إنشاء ملف JSON للاستبدال.
    """)
    file_path0 = './Appの运行に使用する各类文件/قائمة الجذور الإسبرانتية مع الترجمات العربية وتعليقات روبي.csv'
    with open(file_path0, "rb") as file:
        st.download_button(
            label="تنزيل ملف CSV نموذجي رقم 1 (جذور إسبرانتو – ترجمة عربية + روبي)",
            data=file,
            file_name="جذورإسبرانتو_ترجمة_عربية_روبي.csv",
            mime="text/csv"
        )

    st.markdown("""
    **ملف CSV نموذجي رقم 2 (قائمة الجذور الإسبرانتية والأحرف الصينية، وفق خطة Mingeo)**  
    هذا ملف CSV يربط كل جذر إسبرانتو بالأحرف الصينية.
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        st.download_button(
            label="تنزيل ملف CSV نموذجي رقم 2 (جذور إسبرانتو – الأحرف الصينية لمقترح Mingeo)",
            data=file,
            file_name="جذورإسبرانتو_الأحرف_الصينية_Mingeo.csv",
            mime="text/csv"
        )

    st.markdown("""
    **ملف CSV نموذجي رقم 3 (جذور إسبرانتو – الأحرف الصينية)**  
    ملف CSV إضافي يحتوي على ربط بين الجذور الإسبرانتية والأحرف الصينية.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        st.download_button(
            label="تنزيل ملف CSV نموذجي رقم 3 (جذور إسبرانتو – الأحرف الصينية)",
            data=file,
            file_name="جذورإسبرانتو_الأحرف_الصينية.csv",
            mime="text/csv"
        )

    st.markdown("""
    **ملف JSON نموذجي رقم 1 (إعدادات المستخدم لتجزئة الجذور الإسبرانتية)**  
    **الاستخدام**: لتحديد كيفية تجزئة الكلمات في إسبرانتو إلى جذور،
    وترتيب استبدالها، ومواضع إدراج اللواحق (مثل التصريفات الفعلية) وغيرها.  
    (مثل: `["am", "dflt", ["verbo_s1"]]`)
    """)
    json_file_path = './Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json'
    with open(json_file_path, "rb") as file_json:
        st.download_button(
            label="تنزيل ملف JSON نموذجي رقم 1 (إعدادات تجزئة الجذور الإسبرانتية)",
            data=file_json,
            file_name="إعدادات_تجزئة_جذور_إسبرانتو.json",
            mime="application/json"
        )

    st.markdown("""
    **ملف JSON نموذجي رقم 2 (إعدادات المستخدم للنص المستبدل)**  
    **الاستخدام**: يمكن استخدامه لتعيين أحرف صينية أو تنسيق مخصص لبعض الكلمات 
    إضافة إلى بيانات التجزئة المذكورة أعلاه.
    (غالبًا يكفي تعديل ملف CSV وإعدادات JSON لتجزئة الجذور، ولا حاجة لهذا.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        st.download_button(
            label="تنزيل ملف JSON نموذجي رقم 2 (إعدادات النص المستبدل)",
            data=file_json,
            file_name="إعدادات_النص_المستبدل.json",
            mime="application/json"
        )

    st.markdown("""
    **ملف Excel نموذجي رقم 1 (4736 جذر إسبرانتو مع ترجمات/تعليقات روبي في 14 لغة)**  
    (اليابانية، الصينية، الكورية، الإنجليزية، الروسية، الإسبانية، الإيطالية، الفرنسية، الألمانية،
    العربية، الهندية، البولندية، الفيتنامية، الإندونيسية)
    """)
    with open('./Appの运行に使用する各类文件/Kreado de rubenaj komentoj en 14 lingvoj (日本語, 中文, 한국어, English, Русский, español, italiano, français, Deutsch, العربية, हिन्दी, polski, Tiếng Việt, Bahasa Indonesia) respondaj al listo de 4736 Esperant.xlsx', "rb") as file:
        st.download_button(
            label="تنزيل ملف Excel نموذجي رقم 1 (جذور إسبرانتو مع 14 لغة ترجمة)",
            data=file,
            file_name="جذورإسبرانتو_14لغة.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

    st.markdown("""
    **ملف Excel نموذجي رقم 2 (جذور إسبرانتو – ترجمة يابانية مع مستوى التعلم)**  
    **الاستخدام**: مفيد عند تخصيص الجذور الإسبرانتية التي تحتاج لوضع تعليقات أو ترجمة محددة،
    حيث يحتوي على "مستوى التعلم" مشتق من قاموس إسبرانتو-ياباني.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="تنزيل ملف Excel نموذجي رقم 2 (جذور إسبرانتو – ترجمة يابانية مع مستوى التعلم)",
            data=file,
            file_name="جذورإسبرانتو_ترجمةيابانية_مستوىتعلم.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
#=====================================================================
options = {
    'ضبط حجم حروف روبي بتنسيق HTML': 'HTML格式_Ruby文字_大小调整',
    'ضبط حجم حروف روبي بتنسيق HTML (استبدال بالكانجي)': 'HTML格式_Ruby文字_大小调整_汉字替换',
    'تنسيق HTML': 'HTML格式',
    'تنسيق HTML (استبدال بالكانجي)': 'HTML格式_汉字替换',
    'تنسيق الأقواس': '括弧(号)格式',
    'تنسيق الأقواس (استبدال بالكانجي)': '括弧(号)格式_汉字替换',
    'استبدال بسيط': '替换后文字列のみ(仅)保留(简单替换)'
}


display_options = list(options.keys())
selected_display = st.selectbox('اختر صيغة الإخراج:', display_options)
format_type = options[selected_display]

# مثال مختصر لعرض طريقة الاستبدال
...
st.write("---")
st.markdown("**نص مُنسَّق (نموذج تجريبي):**")
...
#=====================================================================
st.header("الخطوة 1: تجهيز ملف CSV")
st.markdown(
    """
### يرجى اختيار **ملف CSV** يحتوي على الجذور الإسبرانتية والمعلومات الترجمية (الأحرف الصينية أو العربية).
...
csv_path_default = "./Appの运行に使用する各类文件/قائمة الجذور الإسبرانتية مع الترجمات العربية وتعليقات روبي.csv"

CSV_data_imported = None

if csv_choice == "رفع ملف CSV":
    st.write("يرجى رفع ملف CSV (يُفضل ترميز UTF-8).")
    ...
    if uploaded_file is not None:
        ...
        st.warning("لم يتم رفع أي ملف CSV.")
        st.stop()

elif csv_choice == "استخدام الملف الافتراضي":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("سيتم استخدام الملف الافتراضي لملف CSV.")
    except FileNotFoundError:
        st.error("تعذر العثور على ملف CSV الافتراضي. تم إيقاف العملية.")
        st.stop()

st.write("تم تحميل بيانات ملف CSV. يمكنك المتابعة للخطوة التالية.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("الخطوة 2: تحضير ملف/ملفات JSON (لتجزئة الجذور وما شابه)")
st.markdown("""
يمكنك رفع أو استخدام بشكل افتراضي ملفات JSON
التي تحتوي على **قواعد تجزئة الجذور الإسبرانتية** و/أو
**النص المستبدل المخصص**.
يمكنك تنزيل ملف JSON نموذجي وتحريره إذا احتجت تخصيصًا إضافيًا.
""")
...
if json_choice == "رفع ملف JSON":
    uploaded_json = st.file_uploader("ارفع ملف JSON", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("تم رفع ملف JSON بنجاح.")
    else:
        st.warning("لم يتم رفع أي ملف JSON.")
        st.stop()
elif json_choice == "استخدام الملف الافتراضي":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("سيتم استخدام ملف JSON الافتراضي (لتجزئة الجذور).")
    except FileNotFoundError:
        st.error("تعذر العثور على ملف JSON الافتراضي.")
        st.stop()
...
if json_choice2 == "رفع ملف JSON":
    uploaded_json = st.file_uploader("ارفع ملف JSON الثاني", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("لم يتم رفع أي ملف JSON.")
        st.stop()
elif json_choice2 == "استخدام الملف الافتراضي":
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            user_replacement_item_setting_list = json.load(g)
        st.info("سيتم استخدام ملف JSON الافتراضي الثاني.")
    except FileNotFoundError:
        st.error("تعذر العثور على ملف JSON الافتراضي الثاني.")
        st.stop()
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("الخطوة 3: إعدادات متقدمة (المعالجة المتوازية)")
with st.expander("انقر هنا لإعدادات المعالجة المتوازية"):
    st.write("""
    يمكنك تحديد عدد العمليات (Processes) المستخدمة للمعالجة المتوازية
    أثناء إنشاء ملف JSON للاستبدال.
    إذا كان حجم النص أو عدد الجذور كبيرًا،
    قد يؤدي تشغيل عدة عمليات إلى تسريع المعالجة بالاستفادة من تعدد الأنوية.
    """)

    use_parallel = st.checkbox("تفعيل المعالجة المتوازية", value=False)
    num_processes = st.number_input("عدد العمليات المتوازية", min_value=2, max_value=6, value=5, step=1)

st.write("### إنشاء ملف JSON النهائي للاستبدال (زر)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("إنشاء ملف JSON للاستبدال"):
    with st.spinner("جاري إنشاء ملف JSON للاستبدال... يرجى الانتظار قليلًا."):
        #-------------------------------------------------------------
        ...
        #     「エスペラント語根 → (汉字 or アラビア語訳など)」を反映させる
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (6) 実際に safe_replace などで大量データを置換
        #-------------------------------------------------------------
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} عنصر قيد المعالجة...")

            progress_bar.progress(100)
            progress_text.write("تم إكمال المرحلة الأطول بنسبة 100٪. (قد يستغرق بضع ثوانٍ إضافية.)")
        ...
        # هنا سيتم دمج البيانات في combined_data وما إلى ذلك... (مختصر)

        download_data = json.dumps(combined_data, ensure_ascii=False, indent=2)
        st.success("تم الانتهاء من توليد قائمة الاستبدال!")

        st.download_button(
            label="تنزيل قائمة الاستبدال النهائية (دمج 3 ملفات JSON)",
            data=download_data,
            file_name="قائمة_الاستبدال_النهائية_دمج3ملفاتJSON.json",
            mime='application/json'
        )